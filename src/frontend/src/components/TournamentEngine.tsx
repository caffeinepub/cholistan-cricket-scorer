...
function generatePools(teamIds: string[]): Pool[] {
  const count = teamIds.length;
  let numPools = 2;
  if (count >= 16) numPools = 4;
  else if (count >= 12) numPools = 3;
  else if (count >= 6) numPools = 2;
  const poolNames = ["A", "B", "C", "D"];
  const pools: Pool[] = Array.from({ length: numPools }, (_, i) => ({
    id: `pool_${poolNames[i]}`,
    name: poolNames[i],
    teamIds: [],
  }));
  // Distribute evenly in snake order
  teamIds.forEach((tid, i) => {
    const poolIdx = i % numPools;
    pools[poolIdx].teamIds.push(tid);
  });
  return pools;
}

function generateRoundRobin(pools: Pool[]): TPoolMatch[] {
...
export function TournamentEngine({
  ...
  const [data, setData] = useState<EngineData>(() => {
    try {
      ...
      if (saved) return JSON.parse(saved) as EngineData;
    } catch {}
    return EMPTY_ENGINE;
  });
  ...
  function toggleTeamSelection(teamId: string) {
    const sel = data.selectedTeamIds;
    if (sel.includes(teamId)) {
      save({ selectedTeamIds: sel.filter((id) => id !== teamId) });
    } else {
      save({ selectedTeamIds: [...sel, teamId] });
    }
  }

  function selectAllTeams() {
    save({ selectedTeamIds: allTeams.map((t) => t.id) });
  }
  ...
  function createTournament() {
    ...
    const pools = generatePools(data.selectedTeamIds);
    const poolMatches = generateRoundRobin(pools);
    save({
      pools,
      poolMatches,
      knockoutMatches: [],
  ...
  // Qualified teams (top 2 per pool)
  ...
  function generateKnockoutStage() {
    ...
    const kms = generateKnockout(data.pools, allStandings);
        ...
        <AnimatePresence mode="wait">
          {tab === "setup" && (
            ...
            >
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-white/60 text-xs font-semibold uppercase tracking-wider">
                    Select Teams ({data.selectedTeamIds.length} selected)
                  </span>
                  <button
                    type="button"
                ...
                <div className="grid grid-cols-2 gap-2">
                  {allTeams.map((team) => {
                    const selected = data.selectedTeamIds.includes(team.id);
                    return (
                      <button
                        key={team.id}
                        type="button"
                        onClick={() => toggleTeamSelection(team.id)}
                        className={`px-3 py-2.5 rounded-xl text-xs font-semibold text-left cursor-pointer transition-all border ${
                          selected
              ...
              {/* Pool preview */}
              {data.selectedTeamIds.length >= 4 && (
                <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                  <p className="text-white/50 text-xs mb-2">
                    {data.selectedTeamIds.length} teams →{" "}
                    {generatePools(data.selectedTeamIds).length} pools
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {generatePools(data.selectedTeamIds).map((pool) => (
                      <span
                        key={pool.id}
                        className="text-xs text-white/70 bg-white/10 px-2 py-1 rounded-lg"
                      >
                        Pool {pool.name}: {pool.teamIds.length} teams
                      </span>
                    ))}
                  </div>
                </div>
              ...
              <button
                ...
                disabled={data.selectedTeamIds.length < 4}
          ...
          {tab === "pools" && (
            ...
            >
              ...
              {data.pools.map((pool) => {
                ...
                return (
                  <div key={pool.id}>
                    <h2 className="text-primary font-bold text-sm tracking-wider mb-3 uppercase">
                      Pool {pool.name}
                    </h2>
          ...
          {tab === "table" && (
            ...
            >
              ...
              {data.pools.map((pool) => {
                const standings = allStandings.get(pool.id) ?? [];
                return (
                  <div key={pool.id}>
                        ...
                        <tbody>
                          {standings.map((s, i) => (
                            ...
                            >
                              ...
                              <td
                                className={`px-2 py-2 text-center font-mono text-[10px] ${
                                  s.nrr >= 0 ? "text-green-400" : "text-red-400"
                                }`}
                              >
                                {s.nrr >= 0 ? "+" : ""}
                                {s.nrr.toFixed(2)}
